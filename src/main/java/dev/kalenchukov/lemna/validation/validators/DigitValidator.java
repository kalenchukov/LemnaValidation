...
public final class DigitValidator extends AbstractValidator
{
	 ...
	 */
	private boolean isValidInteger(@NotNull final Digit constraint, @NotNull final Integer value)
	{
		Objects.requireNonNull(constraint);
	 ...
	 * @return {@code true}, если значение поля корректно, иначе {@code false}.
	 */
	private boolean isValidLong(@NotNull final Digit constraint, @NotNull final Long value)
	{
		Objects.requireNonNull(constraint);
		Objects.requireNonNull(value);
	 ...
	 * @return {@code true}, если значение поля корректно, иначе {@code false}.
	 */
	private boolean isValidShort(@NotNull final Digit constraint, @NotNull final Short value)
	{
		Objects.requireNonNull(constraint);
		Objects.requireNonNull(value);
	 ...
	 * @return {@code true}, если значение поля корректно, иначе {@code false}.
	 */
	private boolean isValidByte(@NotNull final Digit constraint, @NotNull final Byte value)
	{
		Objects.requireNonNull(constraint);
		Objects.requireNonNull(value);
	...
	private boolean isValidAbstract(@NotNull final Digit constraint, @NotNull final String value)
	{
		...
		if (value.length() < constraint.min())
		{
			this.setMessage(StringFormat.format(
				constraint.message(),
				"DEFAULT_MESSAGE",
				this.localeViolations.getString("90026")
			));
		...
		if (value.length() > constraint.max())
		{
			this.setMessage(StringFormat.format(
				constraint.message(),
				"DEFAULT_MESSAGE",
				this.localeViolations.getString("90027")
			));
