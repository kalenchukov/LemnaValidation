...
{
	/**
	 * Конструктор для {@code MacAddressValidator}.
	 * @param locale локализация.
	 */
	public MacAddressValidator(@NotNull final Locale locale)
	{
	...
	public Violating valid(@NotNull final Field field, @Nullable final Object value)
	{
		...
		MacAddress constraint = field.getDeclaredAnnotation(MacAddress.class);
	 ...
	 */
	private boolean isValid(@NotNull final Field field, @NotNull final MacAddress constraint, @Nullable final Object value)
	{
		Objects.requireNonNull(field);
	 ...
	 */
	private boolean isValidString(@NotNull final MacAddress constraint, @NotNull final String value)
	{
		Objects.requireNonNull(constraint);
		Objects.requireNonNull(value);

		if (constraint.ignoreCase() && StringRegexp.isMacAddressIgnoreCase(value)) {
			return true;
		}

		if (!constraint.ignoreCase() && StringRegexp.isMacAddress(value)) {
			return true;
		}
		...
		this.setMessage(StringFormat.format(
			constraint.message(),
			"DEFAULT_MESSAGE",
			this.localeViolations.getString("90028")
		));
