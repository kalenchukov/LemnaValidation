...
/**
 * Класс проверяющего для ограничения {@link DayOfWeek}.
 */
public final class DayOfWeekValidator extends AbstractValidator
{
	/**
	...
	public Violating valid(@NotNull final Field field, @Nullable final Object value)
	{
		...
		DayOfWeek constraint = field.getDeclaredAnnotation(DayOfWeek.class);
		...
		if (!valid)
		{
			...
			this.setParam("MIN", String.valueOf(constraint.format().getFirstDay()));
			this.setParam("MAX", String.valueOf(constraint.format().getLastDay()));
	 ...
	 */
	private boolean isValid(@NotNull final Field field, @NotNull final DayOfWeek constraint, @Nullable final Object value)
	{
		Objects.requireNonNull(field);
	 ...
	 */
	private boolean isValidByte(@NotNull final DayOfWeek constraint, @NotNull final Byte value)
	{
		Objects.requireNonNull(constraint);
	 ...
	 * @return {@code true}, если значение поля корректно, иначе {@code false}.
	 */
	private boolean isValidShort(@NotNull final DayOfWeek constraint, @NotNull final Short value)
	{
		Objects.requireNonNull(constraint);
		Objects.requireNonNull(value);
	 ...
	 * @return {@code true}, если значение поля корректно, иначе {@code false}.
	 */
	private boolean isValidInteger(@NotNull final DayOfWeek constraint, @NotNull final Integer value)
	{
		Objects.requireNonNull(constraint);
		Objects.requireNonNull(value);
	 ...
	 * @return {@code true}, если значение поля корректно, иначе {@code false}.
	 */
	private boolean isValidLong(@NotNull final DayOfWeek constraint, @NotNull final Long value)
	{
		Objects.requireNonNull(constraint);
		Objects.requireNonNull(value);
	 ...
	 * @return {@code true}, если значение поля корректно, иначе {@code false}.
	 */
	private boolean isValidAbstract(@NotNull final DayOfWeek constraint, @NotNull final Long value)
	{
		Objects.requireNonNull(constraint);
		Objects.requireNonNull(value);

		if (value < constraint.format().getFirstDay())
		{
		...
		if (value > constraint.format().getLastDay())
		{
			this.setMessage(StringFormat.format(
				constraint.message(),
				"DEFAULT_MESSAGE",
